...
For this project's purposes, the file is used to make .jpg files available to import into components.
...
This file is used as a base for webpack settings that are the extended by the `webpack.dev.js` and `webpack.prd.js` files (see below).
...
Here is a possible article for the MiniCssExtractPlugin:
...
This plugin is currently used to copy publically available files like favicon images that are not otherwise processed by webpack to the final webpack destination.
...
This webpack configuration file is used with running `npm run dev` as specified in `package.json`.
...
You can find more information about the `webpack.prd.js` file and its options at [Webpack's Production](https://webpack.js.org/guides/production/).
